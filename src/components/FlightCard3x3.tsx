...
export default function FlightCard3x3({
  ...
  // Additive airport overlay from backend.
  ...
  const depAirport = depCode.toUpperCase();
  const arrAirport = arrCode.toUpperCase();
...
const statusTextColor = useAirportOverlayStatus
  ...
  const effectiveAircraftTypeCode =
    toNonEmptyString(f.ac_typecode) ||
    toNonEmptyString(airportOverlay?.ac_icao_type) ||
    toNonEmptyString(airportOverlay?.ac_iata_type) ||
    toNonEmptyString(airportOverlay?.ac_code);

  const mappedAircraftType = aircraftDisplayFromTypecode(effectiveAircraftTypeCode);

  const typeDisplay =
    mappedAircraftType !== "N/A"
      ? mappedAircraftType
      : toNonEmptyString(airportOverlay?.ac_typename) || "N/A";

  const regDisplay =
    toNonEmptyString(f.ac_reg) ||
    toNonEmptyString(airportOverlay?.ac_reg) ||
    "N/A";
        ...
        <div className="flightCard-row flightCard-rowSpaced">
          ...
          <div className="flightCard-cell flightCard-centerCell">
            <div className="flightCard-timeRow">
              ...
              <div className={`flightCard-time ${isCancelled ? "flightCard-timeCancelled" : ""}`}>
                {String(arrTimeDisplay)} LT{arrIsNextDay ? <sup className="flightCard-nextDay">+1</sup> : null}
              </div>
            </div>
          </div>
